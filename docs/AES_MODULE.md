...
- **Hardware-vs-software side channels.** AES-NI and ARMv8 `AESE` / `AESD` are constant-time by construction. The software fallback uses 256-byte S-box lookups and is therefore exposed to cache-timing attacks (Prime+Probe, Flush+Reload, Spectre-class observers). `aes_common_internal.hpp` carries a `@warning` to that effect. Verify hardware AES is available at runtime if you care about side channels — runtime CPUID/`__ARM_FEATURE_AES` detection is done internally but is not surfaced as a public predicate.
