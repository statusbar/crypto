...
/// Per RFC 5297, maximum plaintext size is 2^36 bytes (64 GB); larger plaintexts
/// are rejected and return an empty tag. This limit arises from the 32-bit CTR
/// counter (blocks) combined with 16-byte block size: 2^32 blocks * 16 bytes/block.
/// @param key 64-byte combined SIV key (K1 || K2).
/// @param plaintext_to_ciphertext Buffer encrypted in place (must be <= 2^36 bytes).
/// @param aad Additional authenticated data (not encrypted).
/// @return 16-byte SIV (synthetic initialization vector / authentication tag), or empty array if plaintext exceeds 2^36 bytes.
...
/// is zeroed and false is returned. Maximum ciphertext size is 2^36 bytes (64 GB);
/// larger ciphertexts are rejected.
/// @param key 64-byte combined SIV key (K1 || K2).
/// @param ciphertext_to_plaintext Buffer decrypted in place (must be <= 2^36 bytes).
