...
// libFuzzer harness for AES-128-SIV encrypt/decrypt round-trip (RFC 5297)
...
{
    if (size < 32) {
        return 0;  // 32-byte SIV key (K1 || K2)
    }
    Aes128SivKey key{};
    ...
    auto siv = aes128_siv_encrypt(key, buffer, aad);
    bool ok = aes128_siv_decrypt(key, buffer, std::span<uint8_t const, 16>(siv), aad);
    ...
    if (!aad.empty()) {
        ...
        auto siv2 = aes128_siv_encrypt(key, buffer2, aad);
        bool ok2 = aes128_siv_decrypt(key, buffer2, std::span<uint8_t const, 16>(siv2), std::span<uint8_t const>{});
        if (ok2) {
            __builtin_trap();
        }
    }
    ...
    if (!plaintext.empty()) {
        ...
        auto siv3 = aes128_siv_encrypt(key, buffer3, aad);
        siv3[0] ^= 0x01;
        bool ok3 = aes128_siv_decrypt(key, buffer3, std::span<uint8_t const, 16>(siv3), aad);
        if (ok3) {
            __builtin_trap();
        }
    }
    ...
    if (size >= 48) {
        ...
        span_copy(fuzz_siv, std::span<uint8_t const>(data + 32, 16));
        std::vector<uint8_t> fuzz_ct(data + 48, data + size);
        auto fuzz_ok = aes128_siv_decrypt(
