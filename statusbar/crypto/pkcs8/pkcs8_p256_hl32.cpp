...
// PKCS#8 and SPKI encoding for NIST P-256 keys — 32-bit reduced-radix backend.
//
// The 32-bit-ALU counterpart of pkcs8_p256.cpp: identical orchestration. The
// DER/ASN.1 work and the high-level p256_keypair_from_scalar are already
// backend-agnostic; only the two P-256 point primitives used to validate an
// imported public key are retargeted to p256x32_*. Provides the same
// spki_*_p256 / pkcs8_*_p256 symbols; selected when __int128 is unavailable.
...
auto spki_import_p256(span<uint8_t const> der) -> std::optional<P256PublicKey>
{
    ...
    auto point = p256x32_decode_point_uncompressed(pk.data);
    if (!point || !p256x32_point_on_curve(*point)) {
        return std::nullopt;
    }
