...
// ECDSA over NIST P-256 with SHA-256 — 32-bit reduced-radix backend.
//
// The 32-bit-ALU counterpart of p256_ecdsa.cpp: identical orchestration —
// including the RFC 6979 deterministic-nonce HMAC-DRBG, which uses only
// SHA-256 — but the point and scalar arithmetic runs over the p256x32 /
// p256_sc32 reduced-radix code instead of the 4x64-bit __int128
// implementation. Provides the same p256_* / p256_ecdsa_* symbols;
...
{
    auto d = p256_sc32_from_bytes(scalar_bytes);
    ...
    // Compute Q = d * G
    auto Q_jac = p256x32_scalar_mult_base(d);
    auto Q = p256x32_point_to_affine(Q_jac);
    auto Q_bytes = p256x32_encode_point_uncompressed(Q);
...
{
    auto [d, Q] = p256x32_keypair_from_seed(seed);

    P256PrivateKey sk{};
    SecureArray<32> const d_bytes = p256_sc32_to_bytes(d);
    span_copy(sk.data, d_bytes);

    auto q_encoded = p256x32_encode_point_uncompressed(Q);
    ...
    -> P256Scalar32
{
    ...
    SecureArray<32> h1_octets{};
    {
        ...
        h1_octets = p256_sc32_to_bytes(p256_sc32_reduce_wide(wide));
    }
    ...
    for (int attempt = 0; attempt < 100; ++attempt) {
        ...
        // when int(V) is in [1, n-1]. p256_sc32_from_bytes does NOT reduce, so
        ...
        auto k = p256_sc32_reduce_wide(wide);
        if (!p256_sc32_is_zero(k)) {
            auto k_bytes = p256_sc32_to_bytes(k);
...
auto p256_ecdsa_sign(P256PrivateKey const& sk, span<uint8_t const> message) -> P256EcdsaSignature
{
    ...
    auto e = p256_sc32_from_bytes(e_hash);
    ...
    // Step 4: R = k * G
    auto R_jac = p256x32_scalar_mult_base(k);
    auto R_aff = p256x32_point_to_affine(R_jac);

    // Step 5: r = R.x mod n
    auto r_bytes = p256_fe32_to_bytes(R_aff.x);
    auto r = p256_sc32_from_bytes(r_bytes);
    // Note: p256_sc32_from_bytes doesn't reduce mod n. We need to check if R.x >= n
    // and reduce. Since p > n, R.x could be >= n. Use reduce_wide with zero-padded value.
    {
        ...
        r = p256_sc32_reduce_wide(wide);
    }

    if (p256_sc32_is_zero(r)) {
        return P256EcdsaSignature{};  // Should not happen with valid k
    }

    // Step 6: s = k^(-1) * (e + r * d) mod n
    auto d = p256_sc32_from_bytes(sk.data);
    auto rd = p256_sc32_mul(r, d);
    auto e_plus_rd = p256_sc32_add(e, rd);
    auto k_inv = p256_sc32_inv(k);
    auto s = p256_sc32_mul(k_inv, e_plus_rd);

    if (p256_sc32_is_zero(s)) {
        return P256EcdsaSignature{};  // Should not happen with valid k
    }

    // Step 7: Encode signature as r || s
    P256EcdsaSignature sig{};
    auto r_out = p256_sc32_to_bytes(r);
    auto s_out = p256_sc32_to_bytes(s);
...
auto p256_ecdsa_verify(P256PublicKey const& pk, span<uint8_t const> message, P256EcdsaSignature const& signature) -> bool
{
    ...
    auto r = p256_sc32_from_bytes(r_bytes);
    auto s = p256_sc32_from_bytes(s_bytes);

    // Check r, s in [1, n-1]
    if (p256_sc32_is_zero(r) || p256_sc32_is_zero(s)) {
        return false;
    }

    // Check r < n and s < n (from_bytes doesn't reduce, so check roundtrip)
    {
        auto r_check = p256_sc32_to_bytes(r);
        auto s_check = p256_sc32_to_bytes(s);
    ...
    // Step 3: w = s^(-1) mod n
    auto w = p256_sc32_inv(s);

    // Step 4: u1 = e*w mod n, u2 = r*w mod n
    auto u1 = p256_sc32_mul(e, w);
    auto u2 = p256_sc32_mul(r, w);

    // Step 5: Decode public key
    auto Q_opt = p256x32_decode_point_uncompressed(pk.data);
    if (!Q_opt) {
        return false;
    }
    auto const& Q = *Q_opt;
    ...
    auto R_jac = p256x32_double_scalar_mult(u1, u2, Q);
    if (p256x32_point_is_identity(R_jac)) {
        return false;
    }
    ...
    // Step 7: v = R.x mod n
    auto v_bytes = p256_fe32_to_bytes(R_aff.x);
    P256Scalar32 v{};
    {
        std::array<uint8_t, 2 * p256_scalar_size> wide{};
        span_copy(span(wide).last<p256_scalar_size>(), v_bytes);
        v = p256_sc32_reduce_wide(wide);
    }

    // Step 8: Accept iff v == r
    auto v_out = p256_sc32_to_bytes(v);
    auto r_out = p256_sc32_to_bytes(r);
