...
// Ed25519 sign/verify (RFC 8032 Section 5.1) — 32-bit reduced-radix backend.
//
// The 32-bit-ALU counterpart of ed25519.cpp: identical orchestration, but the
// Edwards-group operations run over ge32_* (10-limb radix-2^25.5 field) and
// the pk->X25519 birational map over fe25519x32_*, instead of the 5x51-bit
// __int128 implementation. The scalar-mod-L arithmetic (sc_reduce /
// sc_mul_add) is shared — it needs no 128-bit type and lives in the ungated
// ed25519_scalar.cpp. Provides the same ed25519* symbols; selected at compile
// time when __int128 is unavailable.
...
auto ed25519_keypair_from_seed(span<uint8_t const, ed25519_seed_size> seed) -> Ed25519PrivateKey
{
    ...
    auto A = ge32_scalar_mult_base(scalar);

    // 4. Encode public key
    auto pk_bytes = ge32_p3_to_bytes(A);
...
auto ed25519_sign(Ed25519PrivateKey const& sk, span<uint8_t const> message) -> Ed25519Signature
{
    ...
    auto R_point = ge32_scalar_mult_base(r_span);
    auto R_bytes = ge32_p3_to_bytes(R_point);
...
auto ed25519_verify(Ed25519PublicKey const& pk, span<uint8_t const> message, Ed25519Signature const& signature) -> bool
{
    ...
    auto A_opt = ge32_from_bytes(pk_span);
    if (!A_opt) {
        return false;
    }
    auto const& A = *A_opt;
    ...
    // 5. Verify: [S]B == R + [h]A
    //    Compute [S]B - [h]A = [h](-A) + [S]B via ge32_double_scalar_mult_vartime
    //    Then compare encoding with R
    auto neg_A = ge32_p3_neg(A);
    auto h_span = make_const_span(h);
    auto check_point = ge32_double_scalar_mult_vartime(h_span, neg_A, S_span);
    auto check_bytes = ge32_p3_to_bytes(check_point);
...
auto ed25519_pk_to_x25519_pk(Ed25519PublicKey const& ed_pk) -> std::optional<X25519PublicKey>
{
    ...
    auto y = fe25519x32_from_bytes(y_span);

    auto one = fe25519x32_one();

    // denominator = 1 - y
    auto den = fe25519x32_sub(one, y);

    // Reject identity point: if y == 1, then den == 0 and the mapping is undefined.
    // fe25519x32_invert(0) returns 0, which would silently produce an all-zero X25519 key
    // (a low-order point that contributes nothing in ECDH).
    if (fe25519x32_is_zero(den) != 0) {
        return std::nullopt;
    }

    // numerator = 1 + y
    auto num = fe25519x32_add(one, y);

    // u = num * den^(-1)
    auto den_inv = fe25519x32_invert(den);
    auto u = fe25519x32_mul(num, den_inv);

    auto u_bytes = fe25519x32_to_bytes(u);
...
auto ed25519_sk_to_x25519_sk(Ed25519PrivateKey const& ed_sk) -> X25519PrivateKey
{
    ...
    auto x_pk_bytes = curve25519x32_scalar_mult(scalar_span, bp_span);
