...
// DL/ECIES implementation (IEEE 1363a-2004 Section 11.3)
...
auto ecies_decrypt(P256PrivateKey const& sk, span<uint8_t const> input, span<uint8_t> plaintext) -> span<uint8_t const>
{
    ...
    auto eph_point = p256_decode_point_x(v_encoded);
    if (!eph_point) {
    ...
    auto eph_encoded = p256_encode_point_uncompressed(*eph_point);
